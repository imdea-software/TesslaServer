defmodule TesslaServer.Computation.Lifted.MulTest do
  use ExUnit.Case, async: true
  use Timex

  alias TesslaServer.Computation.Lifted.Mul
  ...
  test "Should compute product of latest Events and notify children" do
    ...
    event1_3 = %Event{
      type: :change, timestamp: timestamp3, stream_id: @op1, value: 1
    }
    event1_4 = %Event{
    ...
    event2_3 = %Event{
      type: :change, timestamp: timestamp3, stream_id: @op2, value: 9
    }
    event2_4 = %Event{
    ...
    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp0, value: 2}}
    }
    ...
    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp1, value: 3}
    }}

    GenComputation.send_event(@processor, event2_3)

    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp2, value: 9}
    }}
