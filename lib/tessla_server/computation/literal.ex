defmodule TesslaServer.Computation.Literal do
  ...
  def handle_cast(:update, state) do
    ...
    Enum.each(state.children, &GenComputation.send_event(&1, literal))
    {:noreply, state}
  end
  def handle_cast({:add_child, new_child}, state) do
    new_state = %{state | children: [new_child | state.children]}
    event = %Event{value: state.value, stream_id: state.id, timestamp: :literal}
    GenComputation.send_event(new_child, event)
    {:noreply, new_state}
  end
