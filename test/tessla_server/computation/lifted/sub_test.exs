defmodule TesslaServer.Node.Lifted.SubTest do
  ...
  alias TesslaServer.Computation.Lifted.Sub
  ...
  test "Should substract latest Events and notify children" do
    ...
    event2_3 = %Event{
      type: :change, timestamp: timestamp3, stream_id: @op2, value: 4
    }
    event2_4 = %Event{
    ...
    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp1, value: -2}
    }}

    GenComputation.send_event(@processor, event2_3)

    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp2, value: 0}
    }}
