defmodule TesslaServer.Computation.Lifted.MinTest do
  use ExUnit.Case, async: true
  use Timex

  alias TesslaServer.Computation.Lifted.Min
  ...
  setup do
    ...
    Min.start @processor, [@op1, @op2]
    :ok
  end

  test "Should compute minimum of latest Events and notify children" do
    ...
    event1_3 = %Event{
      type: :change, timestamp: timestamp3, stream_id: @op1, value: 2
    }
    event1_4 = %Event{
      type: :change, timestamp: timestamp4, stream_id: @op1, value: 0
    }

    event2_0 = %Event{
      type: :change, timestamp: timestamp0, stream_id: @op2, value: 2
    }
    event2_1 = %Event{
      type: :change, timestamp: timestamp1, stream_id: @op2, value: 3
    }
    event2_3 = %Event{
      type: :change, timestamp: timestamp3, stream_id: @op2, value: 1
    }
    event2_4 = %Event{
    ...
    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp3, value: 1}
    }}
    ...
    assert_receive {_, {:process,
      %Event{type: :change, timestamp: ^timestamp4, value: 0}
    }}
