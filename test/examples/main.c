...
void log_function_call(const char* file, const char* function) {
  ...
  printf("function_calls:%s:%s %ld %d\n", file, function, tv.tv_sec, tv.tv_usec);
}
void log_variable_change(const char* file, const char* variable, int value) {
  struct timeval tv = GetTimeStamp();
  printf("variable_values:%s:%s %ld %d %d\n", file, variable, tv.tv_sec, tv.tv_usec, value);
}
...
void init_ring_buffer() {
  ...
  log_variable_change(__FILE__, "write_ptr", (int) ring_buffer);
...
void process(char* data){
  log_function_call(__FILE__, __FUNCTION__);
  struct timespec ts;
  ts.tv_sec = 0;
  ts.tv_nsec = 100000;
  ...
  while (!feof(stdin)){
    ...
    if (next_write_ptr != read_ptr) {
      *write_ptr = getc(stdin);
      ...
      log_variable_change(__FILE__, "write_ptr", (int) next_write_ptr);
