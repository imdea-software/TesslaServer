defmodule TesslaServer.Node.Multiply do
  @moduledoc """
  Implements a `Node` that multiplies two event streams
  To do so the `state.options` object has to be initialized with the keys `:factor1` and `:factor2`,
  ...
  def prepare_values(state) do
    %{values: get_factors(state), state: state}
  end
  ...
  def process_values(%{values: values, state: state}) do
    value = List.foldl(values, 1, &(&1.value * &2))
  ...
  @spec get_factors(State.t) :: [Event.t]
  defp get_factors(state) do
    [ History.get_latest_input_of_stream(state.history, state.options.factor1),
      History.get_latest_input_of_stream(state.history, state.options.factor2)
