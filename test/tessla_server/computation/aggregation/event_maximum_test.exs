defmodule TesslaServer.Computation.Aggregation.EventMaximumTest do
  use ExUnit.Case, async: false
  use Timex

  alias TesslaServer.Computation.Aggregation.EventMaximum
  alias TesslaServer.{Event, GenComputation, Registry, Source}

  import System, only: [unique_integer: 0]

  doctest EventMaximum
  ...
  setup do
    ...
    EventMaximum.start @processor, [@op1], %{default: @default_value}
    :ok
  end
  ...
  test "Should take value of new event if it is bigger than previous maximum" do
    ...
    event1 = %Event{timestamp: timestamp1, value: 4, stream_id: @op1}
    event2 = %Event{
      timestamp: timestamp2, value: 6, stream_id: @op1
    }
    event3 = %Event{
      timestamp: timestamp3, value: 6, stream_id: @op1
    }
    event4 = %Event{
